     ...
     <Link to='/'> <img className='w-36' src={assets.logo} alt="" /></Link>
      ...
      <div className="flex items-center gap-6">
        ...
        className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4
          bg-black  text-red-400 aspect-square rounded-full text-[8px]'>
           {getCartCount()} </p>
        </Link>
        }
      ...
      <div className={`absolute top-0 right-0 bottom-0 duration-700 overflow-hidden bg-white transition-all 
        ${visible ? "w-full" :" w-0"} `}>
          {/* if you all menu option center use text-cnter  */}
        <div className="flex flex-col  text-gray-600 duration-700">
            <div onClick={() => setVisible(false)} className="flex gap-4 items-center p-3 cursor-pointer">
                <img  src={assets.dropdown_icon} className='h-4 duration-300 rotate-180' alt="" />
                  <p>back</p>
            </div>
